...
import {
  ...
  Boxes, MessageSquareText, Tag, Menu
} from "lucide-react";
...
const STATUS_FLOW = ["paid", "preparing", "processing", "shipped", "delivered"] as const;
...
type TabKey = "overview" | "products" | "orders" | "inventory" | "coupons" | "delivery" | "store" | "customers" | "reviews" | "analytics" | "notifications";
...
const tabs = [
  ...
  { key: "store" as const, label: "Store Settings", icon: Store },
...
const BusinessDashboard = ({ onClose }: BusinessDashboardProps) => {
  ...
  const analytics = useMemo(() => {
    ...
    const pendingOrders = storeOrders.filter((i) => {
      ...
      return st === "paid" || st === "processing" || st === "preparing";
    }).length;
  ...
  const notifications = useMemo(() => {
    const items: { id: string; icon: any; title: string; description: string; type: "warning" | "info" | "success" }[] = [];
    const pendingCount = storeOrders.filter((i) => {
      const st = (i as any).order?.status;
      return st === "paid";
    }).length;
    if (pendingCount > 0) items.push({ id: "pending", icon: ShoppingBag, title: `${pendingCount} new order${pendingCount > 1 ? "s" : ""}`, description: "Require your attention", type: "warning" });
    analytics.lowStockProducts.forEach((p) => items.push({ id: `low-${p.id}`, icon: AlertTriangle, title: `Low stock: ${p.name}`, description: `Only ${p.stock} left`, type: "warning" }));
    analytics.outOfStockProducts.forEach((p) => items.push({ id: `out-${p.id}`, icon: AlertTriangle, title: `Out of stock: ${p.name}`, description: "Restock needed", type: "warning" }));
    return items;
  }, [storeOrders, analytics]);
      ...
      <div className="fixed inset-0 z-50 bg-background overflow-y-auto">
        <div className="mx-auto max-w-lg min-h-screen pb-20">
          <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm border-b border-border px-4 py-3 flex items-center gap-3">
            <button onClick={onClose} className="rounded-full p-1.5 hover:bg-secondary"><ChevronLeft className="h-5 w-5" /></button>
            <h1 className="font-display text-lg font-bold flex-1">Create Store</h1>
          </div>
  ...
  const renderContent = () => {
    switch (tab) {
      case "overview":
        return <DashboardOverviewTab business={business} analytics={analytics} onTabChange={(t) => setTab(t as TabKey)} />;
              ...
              storeOrders.map((item) => {
                ...
                return (
                  <div key={item.id} className="rounded-2xl bg-card border border-border p-4 space-y-3">
                    ...
                    {["paid", "preparing", "processing", "shipped"].includes(status) && (
                      <div className="flex gap-2 flex-wrap">
                        {status === "paid" && (
                          <button onClick={() => updateOrderStatus(item.order_id, "preparing")} className="flex items-center gap-1 rounded-lg px-3 py-1.5 text-[10px] font-bold bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400">
      ...
      case "inventory":
        return <DashboardInventoryTab products={products} onUpdateProduct={updateProduct} />;

      case "coupons":
        return <DashboardCouponsTab businessId={business.id} />;

      case "delivery":
        return <DashboardDeliveryTab business={business as any} onUpdate={updateBusiness} />;

      case "customers":
        return <DashboardCustomersTab businessId={business.id} />;

      case "reviews":
        return <DashboardReviewsTab businessId={business.id} products={products} />;
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                <div key={n.id} className={`rounded-2xl border p-3 flex items-center gap-3 ${
                  n.type === "warning" ? "bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800" : "bg-card border-border"
                }`}>
                  <div className={`flex h-9 w-9 items-center justify-center rounded-xl shrink-0 ${
                    n.type === "warning" ? "bg-amber-100 dark:bg-amber-900/40 text-amber-600" : "bg-primary/10 text-primary"
                  }`}>
      ...
      {!isMobile && (
        <aside className={`h-full bg-card border-r border-border flex flex-col transition-all duration-200 ${sidebarOpen ? "w-56" : "w-14"}`}>
          ...
          <div className="h-14 flex items-center gap-2 px-3 border-b border-border shrink-0">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold truncate">{business.business_name}</p>
          ...
          <nav className="flex-1 overflow-y-auto py-2 space-y-0.5 px-2">
            {tabs.map((t) => {
              ...
              const badge = t.key === "notifications" && notifCount > 0;
              return (
                <button
                  key={t.key}
