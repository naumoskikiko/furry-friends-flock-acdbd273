...
const ShareStoryModal = ({ storyId, mediaUrl, onClose }: ShareStoryModalProps) => {
  ...
  const handleShare = async (conv: any) => {
    ...
    try {
      await fromTable("messages").insert({
        ...
        sender_id: user.id,
        message_text: `📖 Shared a story`,
        message_type: "story_share",
        metadata: { story_id: storyId, media_url: mediaUrl },
      });
      toast({ title: `Story sent to ${conv.other_user.full_name}` });
      onClose();
    } catch {
    ...
    <div className="fixed inset-0 z-[110] flex items-end justify-center bg-black/60" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <h3 className="font-display font-bold">Share Story</h3>
